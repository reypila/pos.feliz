...
const modelGeneric = require('../models/sales.model');
...
module.exports = {
    ...
    Create: function(req, res, next) {
        ...
        if (!enums.CheckExist(req.body.maker) ||
            !enums.CheckExist(req.body.no_invoice) ||
            !enums.CheckExist(req.body.purchase_date) ||
            !enums.CheckExist(req.body.product_code) ||
            !enums.CheckExist(req.body.barcode) ||
            !enums.CheckExist(req.body.description) ||
            !enums.CheckExist(req.body.quantity)) {
            responseutil.Send(res, enums.HTTP_STATUS_CODE.BAD_REQUEST, '', 'Required property not set', '', '', '');
            next();
        }

        if (!enums.IsNumeric(req.body.purchase_date)) {
            responseutil.Send(res, enums.HTTP_STATUS_CODE.BAD_REQUEST, '', 'purchase_date is incorrect', '', '', '');
            next();
        }
